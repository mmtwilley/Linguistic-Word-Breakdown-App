...
Run `../scripts/bash/check-prerequisites.sh --json --paths-only` from repo root and parse `FEATURE_DIR`, `FEATURE_SPEC`. For single quotes in args like "I'm Groot", use escape syntax: e.g 'I'\''m Groot' (or double-quote if possible: "I'm Groot").
