...
- [ ] T009 Create auth API functions in mobile/src/api/auth.ts: register, login, refresh returning typed AuthResponse or ErrorEnvelope (depends on T005, T007)
- [ ] T010 Create mobile/src/auth/AuthContext.tsx: status machine `bootstrapping → signedOut/signedIn` (data-model.md lifecycle), bootstrap from tokenStorage on mount, signIn(email, password) and signOut() that keep secure store and state in sync (depends on T008, T009)
